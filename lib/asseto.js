(function() {
  ...
  Asseto = (function() {
    ...
    Asseto.prototype.c_amd = function(fpath, cb) {
      var config, self;
      self = this;
      config = JSON.parse(this.buildconf.replace(/-raw"/g, '-min"'));
      config.modules = _(config.modules).map(function(m) {
        m.create = true;
        return m;
      });
      config.baseUrl = '.';
      config.dir = this.scriptOut;
      config.appDir = this.scriptOut;
    ...
    Asseto.prototype.c_testacular = function(f, cb) {
      var conf, json, self;
      self = this;
      ...
      if (json) {
        ...
        conf.baseUrl = "/base/";
        conf.modules = void 0;
        return cb("require.config(" + JSON.stringify(conf) + ");\n\nrequire(['./app/app-test'], function() {\n  window.__testacular__.start();\n});");
      } else {
        return cb();
      }
    };
    ...
    Asseto.prototype.precompileFile = function(fpath, cb) {
      ...
      if (S(fpath).endsWith('build.json')) {
        return this.read(file, function(data) {
          self.buildconf = data;
          return self.c_testacular(file, function(data) {
            var fout;
            if (data) {
    ...
    Asseto.prototype.main1 = function(filter, cb) {
      ...
      return this.precompileFiles(fpaths, function() {
        ...
        data = "require.config(" + JSON.stringify(rconf) + ");\nvar Initializer =\n    function load(fs, cb) {\n        // TODO\n    };";
        return self.write(path.join(self.scriptOut, "main-dev.js"), data, function() {
          data = "var Initializer =\n    function load(fs, cb) {\n        Modernizr.load({\n            load: fs,\n            complete: cb\n        });\n    };";
          return self.write(path.join(self.scriptOut, "main-prod.js"), data, function() {
